...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!candidato_id) {
      return new Response(JSON.stringify({ error: "Dados obrigatórios faltando: candidato_id é necessário." }), { 
        status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
      });
    }
    ...
    if (!template || !template.texto) {
      return new Response(JSON.stringify({ success: false, message: "Template de mensagem não encontrado para esta etapa." }), { 
    ...
    if (candidatoError || !candidato) {
      return new Response(JSON.stringify({ success: false, message: "Candidato não encontrado." }), { 
        status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
      });
    }
    ...
    if (!telefone) {
      return new Response(JSON.stringify({ success: false, message: "Número de WhatsApp não encontrado para este candidato" }), { 
        status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
      });
    }
    ...
    if (cleanPhone.length < 10 || cleanPhone.length > 13) {
      return new Response(JSON.stringify({ success: false, message: "O telefone do candidato é inválido. Formato brasileiro esperado." }), { 
        status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
      });
    }
    ...
    const tituloVaga = Array.isArray(candidato.vagas) ? candidato.vagas[0]?.titulo : (candidato.vagas as any)?.titulo;
    ...
    const sendWhatsAppWithRetry = async (phone: string, message: string, retries = 3, backoff = 2000): Promise<any> => {
      try {
         ...
         if (!response.ok) {
            if (response.status === 503 && retries > 0) {
                console.log(`UAZAPI retornou 503. Tentando novamente em ${backoff}ms... (${retries} tentativas restantes)`);
            ...
            throw new Error(`Erro na UAZAPI: ${response.status} - ${response.statusText} | Detalhes: ${errorDetails}`);
         }         
         return await response.json();
      } catch (error: any) {
         if (retries > 0 && (error.message?.includes('503') || error.message?.includes('fetch'))) {
             console.log(`Falha de conexão. Tentando novamente em ${backoff}ms... (${retries} tentativas restantes)`);
             await new Promise(resolve => setTimeout(resolve, backoff));
             return sendWhatsAppWithRetry(phone, message, retries - 1, backoff * 2);
         }
         throw error;
    ...
    try {
       if (uazapiKey) {
           ...
           isSuccess = responseData?.success === true || responseData?.status === 'success' || !!responseData;
           if (!isSuccess) {
    ...
    if (!isSuccess) {
         ...
         return new Response(JSON.stringify({ 
            success: false, 
            message: "A mensagem não pôde ser enviada via WhatsApp, mas o fluxo continuará. Verifique os logs.",
    ...
    return new Response(JSON.stringify({ error: "Ocorreu um erro interno no servidor ao processar o envio.", detalhes: error.message }), { 
