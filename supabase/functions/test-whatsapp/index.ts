...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (isChatbot) {
      ...
      payloadsToTry = [
        {
          url: `${baseUrl}/messages/sendMenu?instance=${instanceId}`,
          body: {
            number: numWpp,
            type: 'button',
            text: `${message}\n\n${perguntaTexto}`,
            choices: [`${btnSimText}|${btnSimId}`, `${btnNaoText}|${btnNaoId}`],
            footerText: 'Escolha uma das opções acima',
          },
          type: 'interativa',
        },
        {
          url: `${baseUrl}/messages/sendText?instance=${instanceId}`,
    ...
    outerLoop: for (let attempt = 0; attempt <= maxRetries; attempt++) {
      for (const payloadVariant of payloadsToTry) {
        ...
        try {
          ...
          console.error(
            `[test-whatsapp] API Error ${response.status} on ${payloadVariant.url}: ${textBody}`,
          )
