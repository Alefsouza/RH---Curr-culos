...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Configura a URL baseando-se no tipo de mensagem
    // Mensagens de botões modernas geralmente utilizam sendInteractive (Evolution v2/UAZAPI)
    ...
    if (isChatbot) {
      ...
      payload_body = {
        number: numWpp || '',
        // Formato para Interactive Messages (Evolution API v2 / UAZAPI)
        options: {
          delay: 1200,
          presence: 'composing',
        },
        interactiveMessage: {
          ...
          footer: {
            text: 'Responda clicando em um dos botões abaixo',
          },
          action: {
            buttons: [
              {
        ...
        // Fallback: Formato legado (Evolution API v1) mantido para garantir suporte
        title: message || '',
        text: fullText,
        type: 'button',
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        console.log(
          `[test-whatsapp] Attempt ${attempt + 1}: Request took ${duration}ms with status ${response.status}`,
        )

        // Log específico para 405 exigido no Acceptance Criteria
        if (response.status === 405) {
          console.error(
            `[test-whatsapp] 405 Method Not Allowed. URL: ${apiUrl.toString()}, Method: POST, Endpoint: ${endpoint}`,
          )
        }
