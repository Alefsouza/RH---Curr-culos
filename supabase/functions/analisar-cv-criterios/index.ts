...
- detalhes (objeto com pontos_fortes (array), pontos_fracos (array), aderencia (string) e motivo (string, explicação breve sobre a decisão, focando na localização se for reprovado))`;
    ...
    if (localizacoesVaga.length > 0 && raioKm > 0) {
      if (!enderecoCV) {
        ...
        statusFinal = 'nao_qualificado';
        if (!motivoFinal.toLowerCase().includes('localização') && !motivoFinal.toLowerCase().includes('distância') && !motivoFinal.toLowerCase().includes('raio')) {
    ...
    if (existing) {
      ...
      const { data, error: insertError } = await supabaseAdmin
        .from('analises')
        .insert({ candidato_id: cv_id, vaga_id: vaga_id, resultado: statusFinal, detalhes: resultJson.detalhes || {}, user_id: user.id })
        .select()
        .single();
