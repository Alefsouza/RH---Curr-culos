      ...
      if (isIncomingMessage && remoteJid) {
        ...
        if (phoneMatch) {
          ...
          if (!candId) {
            ...
            let shortPhone = searchPhone;
            if (searchPhone.length >= 8) {
              shortPhone = searchPhone.substring(searchPhone.length - 8);
            }
            const likePattern = '%' + shortPhone.split('').join('%') + '%';
            const { data: cands } = await supabase.from('candidatos')
              .select('id, user_id, etapa_id')
              .ilike('telefone', likePattern)
              .limit(1)
