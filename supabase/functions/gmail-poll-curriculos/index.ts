  ...
  try {
    ...
    for (const msg of messages) {
      ...
      try {
        ...
        if (selectedPart) {
          attachmentId = selectedPart.body?.attachmentId
        ...
        const storagePath = `${userId}/gmail-${msgId}-${Date.now()}${extension}`
        ...
        const analyzeRes = await supabase.functions.invoke('analyze-resume', {
          body: {
            ...
            nome: '', // Prioritize AI extraction or fallback to 'Candidato Desconhecido'
            email: senderEmail,
            telefone: '',
            vaga_id: null,
            user_id: userId,
