...
BEGIN
  ...
  -- Add respostas_whatsapp to publication if not already present
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables 
    WHERE pubname = 'supabase_realtime' AND schemaname = 'public' AND tablename = 'respostas_whatsapp'
  ) THEN
    ALTER PUBLICATION supabase_realtime ADD TABLE public.respostas_whatsapp;
  END IF;
